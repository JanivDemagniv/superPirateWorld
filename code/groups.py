        ...
        for sprite in sorted(self, key = lambda sprite: sprite.z ):
            if sprite.z < Z_LAYERS['main']:
                if sprite.z == Z_LAYERS['path']:
                    if sprite.level <= self.data.unlock_level:
...
class AllSprites(pygame.sprite.Group):
    def __init__(self, width, height, clouds ,horizon_line ,bg_tile = None,top_limit = 0):
        ...
        self.borders = {
            ...
            'bottom': -self.height + WINDOW_HIEGHT,
            'top': top_limit
        }
    ...
    def draw_sky(self):
        self.dispaly_surface.fill('#ddc6a1')
        horizon_pos = self.horizon_line + self.offset.y

        sea_rect = pygame.FRect(0,horizon_pos,self.width,self.height - horizon_pos)
        pygame.draw.rect(self.dispaly_surface,'#92a9ce',sea_rect)

        #horizon line
        pygame.draw.line(self.dispaly_surface,'#f5f1de',(0,horizon_pos),(WINDOW_WIDTH,horizon_pos),4)
    ...
    def draw_large_cloud(self,dt):
        ...
        for cloud in range(self.large_cloud_tiles):
            ...
            self.dispaly_surface.blit(self.large_cloud,(left,top))
