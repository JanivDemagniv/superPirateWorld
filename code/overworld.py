...
class Overworld:
    ...
    def setup(self,tmx_map,overworld_frames):
        ...
        for obj in tmx_map.get_layer_by_name('Nodes'):
            ...
            #nodes
            if obj.name == 'Node':
                avilable_paths = {k:v for k,v in obj.properties.items() if k in ('left','right','up','down')}
                Node(
                    pos = (obj.x,obj.y),
        ...
        for key,path in path_tiles.items():
            for index , tile in enumerate(path):
                if index > 0 and index < len(path) - 1:
                    perv_tile = path[index - 1] - tile
                    next_tile = path[index + 1] - tile

                    if perv_tile.x == next_tile.x:
                        surf = self.path_frames['vertical']
                    elif perv_tile.y == next_tile.y:
                        surf = self.path_frames['horizontal']
                    else:
                        if perv_tile.x == -1 and next_tile.y == -1 or perv_tile.y == -1 and next_tile.x == -1:
                            surf = self.path_frames['tl']
                        elif perv_tile.x == 1 and next_tile.y == 1 or perv_tile.y == 1 and next_tile.x == 1:
